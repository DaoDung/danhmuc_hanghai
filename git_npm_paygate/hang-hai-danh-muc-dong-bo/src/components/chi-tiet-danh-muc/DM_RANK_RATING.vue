      ...
      <v-layout row wrap>
        <v-flex xs12>
          <div class="title-chitiet">
            <strong class="title-chitiet-danh-muc">Thông tin Chức danh thuyền viên</strong>
          </div>
        </v-flex>
        <v-flex xs12>
          <form class="form-chi-tiet-danh-muc">
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Mã chức danh thuyền viên:</label>
                </v-flex>
                <v-flex xs12 md3>
                  <v-text-field
                    v-model="categoryModel.rankCode"
            ...
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Tên chức danh thuyền viên:</label>
                  <span class="red--text">(*)</span>
                </v-flex>
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.rankNameVN"
                    :readonly="this.$route.query.aticon === 'chi-tiet-danh-muc'"
                    height="25"
                  ></v-text-field>
                </v-flex>
            ...
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Tên chức danh thuyền viên (Tiếng Anh):</label>
                  <span class="red--text">(*)</span>
                </v-flex>
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.rankName"
                    :readonly="this.$route.query.aticon === 'chi-tiet-danh-muc'"
                    height="25"
                  ></v-text-field>
                </v-flex>
    ...
    getDetailCategory() {
      ...
      if (this.$route.query.aticon !== "them-danh-muc") {
        ...
        this.$store.dispatch("category/getDetailCategory", params).then(
          res => {
            vm.categoryModel.rankName = res.rankName;
            vm.categoryModel.rankNameVN = res.rankNameVN;
            vm.categoryModel.rankCode = res.rankCode;
        ...
        vm.categoryModel.rankName = "";
        vm.categoryModel.rankNameVN = "";
        vm.categoryModel.rankCode = "";
