      ...
      <v-layout row wrap>
        <v-flex xs12>
          <div class="title-chitiet">
            <strong class="title-chitiet-danh-muc">Thông tin Loại bao kiện</strong>
          </div>
        </v-flex>
        <v-flex xs12>
          <form class="form-chi-tiet-danh-muc">
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Mã loại bao kiện:</label>
                </v-flex>
                <v-flex xs12 md3>
                  <v-text-field
                    v-model="categoryModel.packageCode"
            ...
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Tên loại bao kiện:</label>
                  <span class="red--text">(*)</span>
                </v-flex>
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.packageNameVN"
                    :readonly="this.$route.query.aticon === 'chi-tiet-danh-muc'"
                    height="25"
                  ></v-text-field>
                </v-flex>
    ...
    getDetailCategory() {
      ...
      if (this.$route.query.aticon !== "them-danh-muc") {
        ...
        this.$store.dispatch("category/getDetailCategory", params).then(
          res => {
            vm.categoryModel.packageCode = res.packageCode;
            vm.categoryModel.packageNameVN = res.packageNameVN;
            if (res.modifiedDate) {
        ...
        vm.categoryModel.packageCode = "";
        vm.categoryModel.packageNameVN = "";
        let date = new Date();
        vm.categoryModel.modifiedDate =
          date.getDate() +
          "/" +
