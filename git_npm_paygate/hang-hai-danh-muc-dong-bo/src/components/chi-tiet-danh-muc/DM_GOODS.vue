      ...
      <v-layout row wrap>
        <v-flex xs12>
          <div class="title-chitiet">
            <strong class="title-chitiet-danh-muc">Thông tin Hàng hóa</strong>
          </div>
        </v-flex>
        <v-flex xs12>
          <form class="form-chi-tiet-danh-muc">
            <v-flex xs12>
              <v-layout align-center>
                ...
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.goodsItemCode"
            ...
            <v-flex xs12>
              <v-layout align-center>
                ...
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.goodsItemName"
                    :readonly="this.$route.query.aticon === 'chi-tiet-danh-muc'"
                    height="25"
                  ></v-text-field>
                </v-flex>
    ...
    getDetailCategory() {
      ...
      if (this.$route.query.aticon !== "them-danh-muc") {
        let params = {
          ...
          goodsItemCode: this.id
        };
        this.$store.dispatch("category/getDetailCategory", params).then(
          res => {
            vm.categoryModel.goodsItemCode = res.goodsItemCode;
            vm.categoryModel.goodsItemNameVN = res.goodsItemNameVN;
            vm.categoryModel.goodsItemName = res.goodsItemName;
            if (res.modifiedDate) {
