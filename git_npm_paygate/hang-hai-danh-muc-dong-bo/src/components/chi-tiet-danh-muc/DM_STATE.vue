      ...
      <v-layout  row wrap >
        <v-flex xs12>
          <div class="title-chitiet">
            <strong class="title-chitiet-danh-muc">Thông tin Quốc gia</strong>
          </div>
        </v-flex>
        <v-flex xs12>
          <form class="form-chi-tiet-danh-muc">
            <v-flex xs12 >
              <v-layout align-center>
                ...
                <v-flex xs12 md3>
                  <v-text-field
                    v-model="categoryModel.stateCode"
            ...
            <v-flex xs12>
              <v-layout align-center>
                ...
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.stateName"
                    :readonly="this.$route.query.aticon === 'chi-tiet-danh-muc'"
                    height="25"
                  ></v-text-field>
                </v-flex>
        ...
        <v-flex xs12 class="text-lg-right">
          <button class="btn-chi-tiet-danhmuc"
            v-if="this.$route.query.aticon !== 'chi-tiet-danh-muc'"
            @click="submit"
          >
            {{btnText}}
          </button>
          <button flat class="btn-chi-tiet-danhmuc"
            @click="back"
          >
...
export default {
    ...
    methods: {
        back () {
            this.$router.push({path: '/danh-sach-danh-muc', query: {categoryId: this.$route.query.categoryId}})
        },
        getDetailCategory () {
            let vm = this
            if (this.$route.query.aticon !== 'them-danh-muc') {
                ...
                this.$store.dispatch('category/getDetailCategory', params)
                    .then(res => {
                        vm.categoryModel.stateName = res.stateName
                        vm.categoryModel.stateCode = res.stateCode
                        vm.categoryModel.description = res.description
                ...
                vm.categoryModel.stateName = ''
                vm.categoryModel.stateCode = ''
                vm.categoryModel.description = ''
                vm.categoryModel.id = 0 
            }
        }
    }
}
