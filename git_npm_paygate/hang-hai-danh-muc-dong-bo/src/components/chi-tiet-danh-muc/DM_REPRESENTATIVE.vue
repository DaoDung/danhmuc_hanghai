      ...
      <v-layout row wrap>
        <v-flex xs12>
          <div class="title-chitiet">
            <strong class="title-chitiet-danh-muc">Thông tin Đại diện cảng vụ</strong>
          </div>
        </v-flex>
        <v-flex xs12>
          <form class="form-chi-tiet-danh-muc">
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Mã đại diện cảng vụ:</label>
                  <span class="red--text">(*)</span>
                </v-flex>
                <v-flex xs12 md3>
                  <v-text-field
                    v-model="categoryModel.repCode"
            ...
            <v-flex xs12>
              <v-layout align-center>
                <v-flex xs12 md4 class="text-sm-left">
                  <label for>Đại diện cảng vụ:</label>
                  <span class="red--text">(*)</span>
                </v-flex>
                <v-flex xs12 md8>
                  <v-text-field
                    v-model="categoryModel.repNameVN"
                    :readonly="this.$route.query.aticon === 'chi-tiet-danh-muc'"
                    height="25"
                  ></v-text-field>
                </v-flex>
    ...
    getDetailCategory() {
      ...
      if (this.$route.query.aticon !== "them-danh-muc") {
        ...
        this.$store.dispatch("category/getDetailCategory", params).then(
          res => {
            vm.categoryModel.repNameVN = res.repNameVN;
            vm.categoryModel.repCode = res.repCode;
        ...
        vm.categoryModel.repCode = "";
        vm.categoryModel.repNameVN = "";
        let date = new Date();
        vm.categoryModel.modifiedDate =
          date.getDate() +
          "/" +
