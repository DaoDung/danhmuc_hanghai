...
const actions = {
  ...
  async updateUserPort ({getters }, queries) {
    return new Promise((resolve, reject) => {
      ...
      let params = {
        departmentCode: queries.departmentCode,
        userId: queries.user.userId,
        status: queries.user.status
      }
      categoryApi.updateUserPort({url: getters.Url.updateUserPort, params: params}, cb, errorCb)
    }) 
  },
}
