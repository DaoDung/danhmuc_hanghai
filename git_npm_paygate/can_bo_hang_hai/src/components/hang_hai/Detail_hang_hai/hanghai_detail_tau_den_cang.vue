    ...
    <v-flex xs12 style="background: #e6e1e1;">
      <v-layout row wrap >
        <v-flex xs6><v-flex style="margin: 8px 0 9px 0; color: #1976d2;" class="text ml-3" xs12>
          <h3>Kế hoạch tàu đến cảng <span @click="showWarning = !showWarning" style="cursor: pointer; color: orange;" v-if="warningTauDen['show']" color="warning">(Cảnh báo)</span></h3> </v-flex>
        </v-flex>
        ...
        <v-card v-if="showWarning" style="width: 100%;">
          <v-card-title class="pt-0 py-0 px-0 adv__search__container">
            <v-alert
              ...
              outline
              >
              <div v-html="item" v-for="(item, index) in warningTauDen['message']"></div>
            </v-alert>
          </v-card-title>
        </v-card>
      </v-layout>
...
export default {
  ...
  methods: {
    ...
    deleteTauDenCang: function () {
      var vm = this
      var cf = confirm('Bạn muốn xóa dữ liệu này!')
      if (cf) {
        let data = {
          vmaItineraryScheduleId: vm.id
        }
        vm.$store.dispatch('deleteTauDenCang', data).then(function (result) {
          if (result.hasOwnProperty('errorCode')) {
    ...
    parseTimeTau: function (modelTauDen) {
      var vm = this
      if (!modelTauDen) {
