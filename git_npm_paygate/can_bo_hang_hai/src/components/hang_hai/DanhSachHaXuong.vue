<template>
    <div>
      ...
      <v-data-table
        :headers="headersDsHaXuong"
        :items="itemsDsHaXuong"
      ...
      >
        <template slot="headers" slot-scope="props">
          ...
          <tr class="" style="border-bottom: 1px solid #ccc;">
            ...
            <th>
              <v-text-field
              v-model="adv.tenTauDSHX"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
            <th>
              <datetime-picker
              v-model="adv.thoiGianDenDSHX"
              :first-day="1"
              :show-dst="false"
              :show-hours="false"
              :show-minutes="false"
            ...
            <th>
              <v-text-field
              v-model="adv.mucDichTX"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
            <th>
              <v-text-field
              v-model="adv.khuVucThaXuong"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
          </tr>
        ...
        <template slot="items" slot-scope="props">
          <td class="text-xs-left" style="padding-top: 5px; width: 2%;">
            {{ loadDataTablePageDSHX * 15 - 15 + props.index + 1 }}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.tenTauDSHX}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.thoiGianDenDSHX}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.thoiGianDiDSHX}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.thoiGianBDTX}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.thoiGianKTTX}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.mucDichTX}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">        
            {{props.item.khuVucThaXuong}}
          </td>
        </template>
      </v-data-table>
      <div class="text-xs-right layout wrap" style="position: relative;">
        <div class="flex pagging-table px-2"> 
          <tiny-pagination :total="totalDsHaXuong" :page="loadDataTablePageDSHX" custom-class="custom-tiny-class" 
...
export default {
  ...
  created () {
    ...
    if (query.hasOwnProperty('page') && query['page'] !== 1) {
      vm.loadDataTablePageDSHX = query['page']
    } else {
      vm.loadDataTablePageDSHX = 1
    }
    let param = null
    vm.loading = true
    param = {
      ...
      start: vm.loadDataTablePageDSHX * 15 - 15,
      end: vm.loadDataTablePageDSHX * 15
    }
    ...
    vm.$store.dispatch('loadDanhSachTauBien', param).then(function (result) {
      vm.itemsDsHaXuong = result.data
      vm.totalDsHaXuong = result.total
      vm.loading = false
    })
  },
  watch: {
