<template>
    <div>
      ...
      <v-data-table
        :headers="headersDsKhangNghi"
        :items="itemsDsKhangNghi"
      ...
      >
        <template slot="headers" slot-scope="props">
          ...
          <tr class="" style="border-bottom: 1px solid #ccc;">
            ...
            <th>
              <v-text-field
              v-model="adv.tenTauDSKN"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
            <th>
              <v-text-field
              v-model="adv.noiDUngDSKN"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
            <th>
              <datetime-picker
              v-model="adv.thoiGianNhanKhangCaoDSKN"
              :first-day="1"
              :show-dst="false"
              :show-hours="true"
              :show-minutes="true"
            ...
            <th>
              <v-text-field
              v-model="adv.daiLyDSKN"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
            <th>
              <v-text-field
              v-model="adv.chuTauDSKN"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
            <th>
              <v-text-field
              v-model="adv.khaiThacDSKN"
              class="px-0 py-0 mx-0 my-0"
              ></v-text-field>
            </th>
          </tr>
        ...
        <template slot="items" slot-scope="props">
          ...
          {{props.item.tenTauDSKN}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.noiDUngDSKN}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.thoiGianNhanKhangCaoDSKN}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.daiLyDSKN}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.chuTauDSKN}}
          </td>
          <td class="text-xs-center" style="padding-top: 5px;">
          {{props.item.khaiThacDSKN}}
          </td>
        </template>
      </v-data-table>
      <div class="text-xs-right layout wrap" style="position: relative;">
        <div class="flex pagging-table px-2"> 
          <tiny-pagination :total="totalDsKhangNghi" :page="loadDataTablePageDSKN" custom-class="custom-tiny-class" 
...
export default {
  ...
  data: () => ({
    ...
    headersDsKhangNghi: [
      ...
      {
        'text': 'Thời gian nhận <br> kháng cáo',
        'align': 'center',
        'sortable': false,
        'id': 'stt'
      },
  ...
  created () {
    ...
    if (query.hasOwnProperty('page') && query['page'] !== 1) {
      vm.loadDataTablePageDSKN = query['page']
    } else {
      vm.loadDataTablePageDSKN = 1
    }
    let param = null
    vm.loading = true
    param = {
      ...
      start: vm.loadDataTablePageDSKN * 15 - 15,
      end: vm.loadDataTablePageDSKN * 15
    }
    ...
    vm.$store.dispatch('loadDanhSachTauBien', param).then(function (result) {
      vm.itemsDsKhangNghi = result.data
      vm.totalDsKhangNghi = result.total
      vm.loading = false
    })
  },
  watch: {
