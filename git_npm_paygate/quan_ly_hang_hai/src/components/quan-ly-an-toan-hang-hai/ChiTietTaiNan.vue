        ...
        <div id="chi-tiet">
          ...
          >
            <v-card>
              ...
              <div style="width:50%; padding: 0 10px; margin-top: 10px;">
                <v-text-field
                  outlined
                  placeholder="Tìm tàu"
                  @change="searchShips"
              ...
              <div id="dialog" style="width:100%; padding: 10px;">
                <table style="width:100%;">
                    ...
                    <tbody>
                      ...
                      <tr v-if="ships.length === 0"><td :colspan="headers.length" style="padding-left: 30%;">Không có dữ liệu</td></tr>
                    </tbody>
                  </table>
              </div>
  ...
  methods: {
    async getDetail () {
      ...
      if (this.action !== 'them-tai-nan') {
        ...
        vm.ngayDieuTra = await vm.formatDate(new Date)
        vm.ngayTaiNan = await vm.formatDate(new Date)
        vm.hanNopBaoCao = await vm.formatDate(new Date)
        vm.taiNanHangHai = {
          id: '',
          portofAuthority: '',
          accidentCode: '',
    ...
    searchShips (val) {
      if(val && val.length > 3){
        let vm = this
        this.$store.dispatch('TaiNanHangHai/getShips', {shipName: val}).then(
          res => {
            vm.ships = res
          }
        )
