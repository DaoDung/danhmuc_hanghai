...
public class BanKhaiKiemDichThucVatUtils {
	...
	public static JSONObject buildThanhPhan(ThemeDisplay themeDisplay, long documentName, int documentYear) {
		...
		TempPlantQuarantine results = TempPlantQuarantineLocalServiceUtil
				.getLastByDocumentNameAndDocumentYear(documentName, documentYear);

		result.put("code", "NC_13");
		result.put("name", LanguageUtil.get(themeDisplay.getLocale(), "ban-khai-kiem-dich-thuc-vat"));
		...
		result.put("messageType", MessageType.BAN_KHAI_KIEM_DICH_THUC_VAT);
		result.put("requestCode", requestCode);

		String icon = checkDisplayIcon(documentName, documentYear, MessageType.BAN_KHAI_KIEM_DICH_THUC_VAT);
	...
	public int _banKhaiKiemDichThucVat(String requestCode, long documentName, int documentYear, int messageType,
		...
		try {
			...
			if (results != null && results.size() > 0) {
				TempPlantQuarantine tempShipSecurityMessage = results.get(0);
				...
				TempPlantQuarantineLocalServiceUtil.updateTempPlantQuarantine(tempShipSecurityMessage);
