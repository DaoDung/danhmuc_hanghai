...
public class TT2JSONProvider {

	private static Log _log = LogFactoryUtil.getLog(TT2JSONProvider.class);
...
//		}
	...
	public static JSONArray getFileThanhPhanVersionList(ThemeDisplay themeDisplay, int messageType, long documentName,
		...
		if (messageType == MessageType.THONG_BAO_TAU_ROI_CANG) {
			...
			result = BanKhaiDanhSachHangHoaNguyHiemUtils.getFileThanhPhanVersionList(themeDisplay, documentName,
					documentYear, messageType);

		} else if (messageType == MessageType.BAN_KHAI_DU_TRU_CUA_TAU) {

			result = BanKhaiDuTruCuaTauUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);

		} else if (messageType == MessageType.BAN_KHAI_HANH_LY_THUYEN_VIEN) {

			result = BanKhaiHanhLyThuyenVienUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);

		} else if (messageType == MessageType.BAN_KHAI_BAO_Y_TE_HANG_HAI) {

			result = BanKhaiBaoYTeHangHaiUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);

		} else if (messageType == MessageType.BAN_KHAI_KIEM_DICH_THUC_VAT) {

			result = BanKhaiKiemDichThucVatUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);

		} else if (messageType == MessageType.BAN_KHAI_KIEM_DICH_DONG_VAT) {

			result = BanKhaiKiemDichDongVatUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);
	...
	public static JSONObject exportPDFDetailThanhPhan(Locale locale, int documentName, int documentYear,
		...
		try {
			...
			if (messageType == MessageType.THONG_BAO_TAU_ROI_CANG) {
				tenFileExport = nanoTime + "_" + ReportConstant.THONG_BAO_TAU_ROI_CANG_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_CHUNG) {
			...
			if ( messageType == MessageType.GIAY_PHEP_ROI_CANG_CUOI_CUNG ||
					(Validator.isNotNull(issuePortClearance) && issuePortClearance.getAttachedFile() > 0 && DocumentUtils.existDLFileAbsPath(issuePortClearance.getAttachedFile()) && messageType == MessageType.GIAY_PHEP_ROI_CANG_BO_GTVT_CAP_KHI_XUAT_CANH)) {
