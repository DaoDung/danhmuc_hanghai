...
//	PTTNDAcceptance
	public PTTNDAcceptance convertXMLPTTNDAcceptance(String requestCode) throws SystemException {
		...
		_log.info("=================Acceptance===DATA GUI MESSAGE===RequestCode====" + requestCode);
		...
		if (object != null) {
			try {
				...
				// SMS
				_log.info("=================Acceptance===DATA GUI MESSAGE===object.getPortCode()====" +object.getPortCode());
				...
				if (tempDocument.getShipTypeCode().equalsIgnoreCase("SMS"))
				{
					...
					_log.info("=================Acceptance===DATA GUI MESSAGE===item.setPortCode====" +item.getPortCode());
				}
				else
				{
					item.setPortCode(String.valueOf(object.getPortCode()));
