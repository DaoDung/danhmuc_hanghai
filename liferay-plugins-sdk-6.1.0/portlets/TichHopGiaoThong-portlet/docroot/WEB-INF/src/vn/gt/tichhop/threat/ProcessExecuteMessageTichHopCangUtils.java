...
public class ProcessExecuteMessageTichHopCangUtils {
	...
	private static Log log = LogFactoryUtil.getLog(ProcessExecuteMessageTichHopCangUtils.class);
...
//			IMTextService imtService = CallWs2HaiQuan.getIMTextSercice();
			...
			if (xml != null && xml.length() > 0) {
				...
				SendMessgaeFunctions.insertMessageQueue(header, xml,
						MessageType.REQUEST_DIRECTION_CV_TO_HQ, null, MessgaePriorityLevels.HIGHT);
			} else {
	...
	private boolean tichHop_ThongBao_TauRoiCang(ObjectExcute objectExcute, Header headerReceive) throws SystemException {
		...
		if (function.equalsIgnoreCase(MessageType.FUNCTION_CHAP_NHAN_HO_SO_CHUNG_TU)) {
			...
			// TODO ---- minhhandsome -- dieu kien thuc tuc xuat canh
			List<TempDocument> documents = TempDocumentLocalServiceUtil.findTemDocumentByDocumentNameAndDocumentYear((new Long(headerReceive
					.getSubject().getReference())).intValue(), headerReceive.getSubject().getDocumentYear());
	...
	// TODO ---- 	// son_vh bo sung 11 ham 20160119 
//	tichHop_GiayPhep_QuaCanh
//	yCauLuuLaiGiayPhepQuaCanh
//	huyGiayPhepQuaCanh
//	tichHop_ThongBao_TauQuaCanh
//	tichHop_XacBao_TauQuaCanh
//	tichHop_KetQua_XemXetChungTuDinhKem
//	tichHop_ThongBao_ChamPheDuyetHTTT
//	tichHop_BAN_KHAI_CHUNG
//	tichHop_BAN_KHAI_DANH_SACH_THUYEN_VIEN
//	tichHop_BAN_KHAI_DANH_SACH_HANH_KHACH
//	tichHop_BAN_KHAI_DANH_SACH_HANG_HOA_NGUY_HIEM
	...
	private boolean tichHop_BAN_KHAI_DANH_SACH_HANG_HOA_NGUY_HIEM(ObjectExcute objectExcute, Header headerReceive) throws SystemException {
		...
		if (function.equalsIgnoreCase(MessageType.FUNCTION_CHAP_NHAN_HO_SO_CHUNG_TU)) {
			...
			List<TempDangerousGoodsNanifest> temps = TempDangerousGoodsNanifestLocalServiceUtil.findByRequestCode(requestCodeOld);
			
			if (Validator.isNotNull(temps) && temps.size() > 0) {
				
				TempDangerousGoodsNanifest temp = temps.get(0);
				temp.setRequestState(TrangThaiHoSo.CHAP_NHAN_BAN_KHAI);
					...
					if(feeNotice != null) {
						...
						if(debitNote == null) {
							TempDebitNoteLocalServiceUtil.doTempDebitNote(documentName, documentYear, corporationid, debitnotenumber, division, financialaccountant, 
									fromdate, markasdeleted, organization, paymenttype, reportby, reportdate, todate, totalpayment, transactionid, document.getMaritimeCode(), feeNotice.getCurrency());
						} else {
							result = false;
						}
					}
