...
public class IssuePortClearanceFinderImpl extends BasePersistenceImpl<IssuePortClearance> implements IssuePortClearanceFinder {
	...
	public IssuePortClearance findByRequestCode(String requestCode) {
		...
		try {
			...
			query.append("SELECT * FROM ISSUE_PORT_CLEARANCE WHERE RequestCode = ? order by id desc");
			...
			// execute the query and return a list from the db
			List<IssuePortClearance> lstPort = (List<IssuePortClearance>) QueryUtil.list(q, getDialect(), QueryUtil.ALL_POS, QueryUtil.ALL_POS);
