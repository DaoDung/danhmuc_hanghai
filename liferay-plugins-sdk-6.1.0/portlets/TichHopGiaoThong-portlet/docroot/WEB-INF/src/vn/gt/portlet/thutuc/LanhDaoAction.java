 ...
 */
public class LanhDaoAction extends TransportationMVCAction {
	...
	private static Log _log = LogFactoryUtil.getLog(LanhDaoAction.class);
	...
	public void getComputerHash(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException {
		_log.info("-----------vao ------------getComputerHash thu tuc----------");
		try {
			...
			IssueShiftingOrder shiftingOrder = null;
			...
			if (messageType == 70) {
				...
				List<IssuePermissionForTransit> results = IssuePermissionForTransitLocalServiceUtil
						.findByDocumentYearAndDocumentYearOrderByColumn(documentName, documentYear, KeyParams.ID,
								KeyParams.ORDER_BY_DESC);
				issuePermissionForTransit = results.get(0);

				fileEntryId = issuePermissionForTransit.getAttachedFile();

				requestCode = issuePermissionForTransit.getRequestCode();
			...
			if (fileEntryId > 0) {
				...
				FileEntry fileEntry = FileUploadUtils.uploadFile(themeDisplay.getUserId(), groupId, fileEntryId, file,
						pathFileSub + tenFileExport, null, null, serviceContext);

				if (messageType == 70) {

					shiftingOrder.setAttachedFile(fileEntry.getFileEntryId());

					IssueShiftingOrderLocalServiceUtil.updateIssueShiftingOrder(shiftingOrder);
				} else if (messageType == 60) {
					...
					issuePermissionForTransit.setAttachedFile(fileEntry.getFileEntryId());
			...
			if (messageType == 70) {
				...
				vn.gt.portlet.kehoach.SignatureUtil.genComputerHashByNoiDungHoSo(resourceRequest, resourceResponse,
						filePath, documentName, documentYear, messageType, groupId, 1);

			} else if (messageType == 60) {

				SignatureUtil.genComputerHashByNoiDungHoSo(resourceRequest, resourceResponse, filePath, documentName,
						documentYear, messageType, groupId, 1);
			} else if (messageType == 90) {
	...
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			...
			if (resourceID.equals("getListPortWharf")) {
				...
				if (advSearch) {
					...
					writeJSON(resourceRequest, resourceResponse,
							JSONProviderUtils.getHistoryTable(themeDisplay, documentTypeCode, maritimeCodeOwner,
									...
									nameDepartureShipAgency, maritimeCodeAdv, shipDateToStart, shipDateToEnd, maBanKhai,
									daily, maritimeCodeNext, start, end, isDTND, isDTNDCam));
					...
					writeJSON(resourceRequest, resourceResponse,
							JSONProviderUtils.getHistoryTable(themeDisplay, documentTypeCode, maritimeCode, portCode,
									...
									nameDepartureShipAgency, portRegionCode, StringPool.BLANK, StringPool.BLANK, maBanKhai,
									daily, maritimeCodeNext, start, end, isDTND, isDTNDCam));
				}
				...
				if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_NHAP_CANH)) {
					writeJSON(resourceRequest, resourceResponse, TT1JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_XUAT_CANH)) {
					writeJSON(resourceRequest, resourceResponse, TT2JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_NHAP_CANH_6)) {
					writeJSON(resourceRequest, resourceResponse, TT16JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_XUAT_CANH_7)) {
					writeJSON(resourceRequest, resourceResponse, TT17JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_QUA_CANH)) {
					writeJSON(resourceRequest, resourceResponse, TT3JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.NHAP_CANH_DAU_KHI)) {
					writeJSON(resourceRequest, resourceResponse, TT4JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.XUAT_CANH_DAU_KHI)) {
					writeJSON(resourceRequest, resourceResponse, TT5JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.VAO_CANH_DAU_KHI)) {
					writeJSON(resourceRequest, resourceResponse, TT6JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.ROI_CANH_DAU_KHI)) {
					writeJSON(resourceRequest, resourceResponse, TT7JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_VAO_CANG)) {
					writeJSON(resourceRequest, resourceResponse, TT8JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.TAU_THUYEN_ROI_CANG)) {
					writeJSON(resourceRequest, resourceResponse, TT9JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.CHUYEN_CANG_VAO)) {
					writeJSON(resourceRequest, resourceResponse, TT10JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.CHUYEN_CANG_ROI)) {
					writeJSON(resourceRequest, resourceResponse, TT11JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.VIET_CAM_NHAP_CANH)) {
					writeJSON(resourceRequest, resourceResponse, TT14JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				} else if (documentType.equalsIgnoreCase(ChuyenDichTrangThaiUtils.VIET_CAM_XUAT_CANH)) {
					writeJSON(resourceRequest, resourceResponse, TT15JSONProvider.checkThanhPhanActionButton(
							themeDisplay, documentName, documentYear, messageType, roleType, requestCode, request));
				}
				...
				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getGiayToXuatTrinh_39(themeDisplay,
						documentTypeCode, documentName, documentYear));
			} else if (resourceID.equals("getGiayToXuatTrinh_40")) {
				String documentTypeCode = ParamUtil.getString(requestOrg, "documentTypeCode", "");
				int documentName = ParamUtil.getInteger(requestOrg, "documentName", 0);

				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getGiayToXuatTrinh_40(themeDisplay,
						documentTypeCode, documentName, documentYear));
			} else if (resourceID.equals("getGiayToXuatTrinh_48")) {
				String documentTypeCode = ParamUtil.getString(requestOrg, "documentTypeCode", "");
				int documentName = ParamUtil.getInteger(requestOrg, "documentName", 0);

				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getGiayToXuatTrinh_48(themeDisplay,
						documentTypeCode, documentName, documentYear));
			} else if (resourceID.equals("getGiayToXuatTrinh_49")) {
				String documentTypeCode = ParamUtil.getString(requestOrg, "documentTypeCode", "");
				int documentName = ParamUtil.getInteger(requestOrg, "documentName", 0);

				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getGiayToXuatTrinh_49(themeDisplay,
						documentTypeCode, documentName, documentYear));
			} else if (resourceID.equals("getGiayToXuatTrinh_50")) {
				String documentTypeCode = ParamUtil.getString(requestOrg, "documentTypeCode", "");
				int documentName = ParamUtil.getInteger(requestOrg, "documentName", 0);

				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getGiayToXuatTrinh_50(themeDisplay,
						documentTypeCode, documentName, documentYear));
				...
				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getMessageType23EXT(themeDisplay,
				...
				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getMessageType60EXT(themeDisplay,
						documentName, documentYear, messageType, roleType, requestCode, request));
				...
				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getMessageType60EXTTable(themeDisplay,
						documentName, documentYear, messageType, roleType, requestCode, request));
				...
				writeJSON(resourceRequest, resourceResponse, JSONProviderUtils.getMessageType90EXT(themeDisplay,
						documentName, documentYear, messageType, roleType, requestCode, request));
