...
public class TT8ActionProvider {

	private static Log _log = LogFactoryUtil.getLog(TT8ActionProvider.class);
	...
	public static JSONObject actionKeHoach(ThemeDisplay themeDisplay, UploadPortletRequest uploadPortletRequest,
		...
		if (chuyenDichStatus != ChuyenDichTrangThaiUtils.CHUYEN_DICH_THAT_BAI) {
			...
			if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
				int state = TT8XuLyNghiepVuUtils.doActionHoSo(documentName, documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(), uploadPortletRequest, actionRequest, Boolean.TRUE);
				// rollback neu xu ly nghiep vu xay ra loi
				if (state == TT8XuLyNghiepVuUtils.XU_LY_THAT_BAI) {
					ChuyenDichTrangThaiUtils.doHoSo(documentName, documentYear, requestState, Boolean.TRUE);
				}
			} else {
				TT8XuLyNghiepVuUtils.doActionBanKhai(documentName, documentYear, actionType, requestState,
...
//			}
	...
	public static JSONObject actionThuTuc(ThemeDisplay themeDisplay, UploadPortletRequest uploadPortletRequest,
		...
		if (chuyenDichStatus != ChuyenDichTrangThaiUtils.CHUYEN_DICH_THAT_BAI) {
			...
			if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
				int state = TT8XuLyNghiepVuUtils.doActionHoSo(documentName, documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(), uploadPortletRequest, actionRequest, Boolean.FALSE);
				// rollback neu xu ly nghiep vu xay ra loi
				if (state == TT8XuLyNghiepVuUtils.XU_LY_THAT_BAI) {
					ChuyenDichTrangThaiUtils.doHoSo(documentName, documentYear, documentStatusCode, Boolean.FALSE);
				}
			} else {
				chuyenDichStatus = TT8XuLyNghiepVuUtils.doActionBanKhai(documentName, documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(), messageType, requestCode, Boolean.FALSE,
						uploadPortletRequest, actionRequest);
			}
