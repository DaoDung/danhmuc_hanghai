...
public class TempShipStoresDeclarationFinderImpl extends BasePersistenceImpl<TempShipStoresDeclaration> implements TempShipStoresDeclarationFinder {
	
	private Log log = LogFactoryUtil.getLog(TempShipStoresDeclarationFinderImpl.class);
	...
	public int countBydocumentNameAnddocumentYear(long documentName, int documentYear) {
		...
		try {
			...
			query.append("SELECT count(*) as total FROM TEMP_SHIP_STORES_DECLARATION WHERE DocumentName = ? and DocumentYear = ? ");
	...
	public List<TempShipStoresDeclaration> findBydocumentNameAnddocumentYear(long documentName, int documentYear) {
		...
		try {
			...
			query.append("SELECT * FROM TEMP_SHIP_STORES_DECLARATION WHERE DocumentName = ? and DocumentYear = ? ");
			
			String sql = query.toString();
			log.debug("=========findBydocumentNameAnddocumentYear========" + sql);
			SQLQuery q = session.createSQLQuery(sql);
			q.setCacheable(false);
			q.addEntity("TempShipStoresDeclaration", TempShipStoresDeclarationImpl.class);
			QueryPos qPos = QueryPos.getInstance(q);
			qPos.add(documentName);
			qPos.add(documentYear);
			
			// execute the query and return a list from the db
			return (List<TempShipStoresDeclaration>) QueryUtil.list(q, getDialect(), QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			closeSession(session);
	...
	public TempShipStoresDeclaration findTempShipStoresDeclarationByRequestCode(String requestCode) throws SystemException {
		...
		try {
			...
			query.append("SELECT * FROM TEMP_SHIP_STORES_DECLARATION WHERE RequestCode = ?");
			...
			// execute the query and return a list from the db
			return (TempShipStoresDeclaration) q.uniqueResult();
		} catch (Exception e) {
...
/*			interfaceRequest.append("SELECT inter.RequestCode FROM interface_request inter INNER JOIN (");
			...
			query.append("SELECT * FROM temp_ship_stores_declaration WHERE DocumentName = ? and DocumentYear = ? order by ID desc");
			...
			// execute the query and return a list from the db
			List<TempShipStoresDeclaration> lstTem = (List<TempShipStoresDeclaration>) QueryUtil.list(q, getDialect(), QueryUtil.ALL_POS,
					QueryUtil.ALL_POS);
