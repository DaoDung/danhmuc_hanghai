...
public class KeyPay {
	...
	public static String getSecureHashResponse(KeyPay keyPay) {
		...
		try {
			...
			if (Validator.isNotNull(keyPay.getMerchant_trans_id())) {
				debitNote = TempDebitNoteLocalServiceUtil
						.getDebitNoteByTransactionId(keyPay.getMerchant_trans_id());
				
				paymentConfig = TempMaritimePaymentConfigLocalServiceUtil.getPaymentConfig(
			...
			if (Validator.isNotNull(paymentConfig)) {
				...
				return hash.hashAllFields(fields, paymentConfig.getMerchantKey());
			}
	...
	public static boolean checkSecureHash(KeyPay keyPay) {
		...
		_log.info("keyPay.merchantSig:" + merchantSig);
		_log.info("keyPay.getSecure_hash():" + keyPay.getSecure_hash());

		if (keyPay.getSecure_hash().equalsIgnoreCase(merchantSig)) {
			return true;
		}
		else {
			return false;
