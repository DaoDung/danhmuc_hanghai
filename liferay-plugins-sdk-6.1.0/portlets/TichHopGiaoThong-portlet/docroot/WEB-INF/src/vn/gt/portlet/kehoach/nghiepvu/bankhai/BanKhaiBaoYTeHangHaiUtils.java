...
public class BanKhaiBaoYTeHangHaiUtils {
	...
	public static JSONObject buildThanhPhan(ThemeDisplay themeDisplay, long documentName, int documentYear) {
		...
		TempDeclarationOfHealth results = TempDeclarationOfHealthLocalServiceUtil.getLastByDocumentNameAndDocumentYear(documentName, documentYear);

		result.put("code", "NC_12");
		result.put("name", LanguageUtil.get(themeDisplay.getLocale(), "giay-khai-bao-y-te-hang-hai"));
		...
		result.put("messageType", MessageType.BAN_KHAI_BAO_Y_TE_HANG_HAI);
		result.put("requestCode", requestCode);

		String icon = checkDisplayIcon(documentName, documentYear, MessageType.BAN_KHAI_BAO_Y_TE_HANG_HAI);
	...
	public int _banKhaiBaoYTeHangHai(String requestCode, long documentName, int documentYear, int messageType,
		...
		try {
			...
			if (results != null && results.size() > 0) {
				...
				tempDeclarationOfHealth.setRequestState(desStatus);
