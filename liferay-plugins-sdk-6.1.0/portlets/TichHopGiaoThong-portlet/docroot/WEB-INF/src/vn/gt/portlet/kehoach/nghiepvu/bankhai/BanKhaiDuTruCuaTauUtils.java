...
public class BanKhaiDuTruCuaTauUtils {
	...
	private static String checkDisplayIcon(long documentName, int documentYear, int businessTypeCode) {
		List<TempShipStoresDeclaration> lstResult = TempShipStoresDeclarationLocalServiceUtil
	...
	public static JSONObject buildThanhPhan(ThemeDisplay themeDisplay, long documentName, int documentYear) {
		...
		TempShipStoresDeclaration results = TempShipStoresDeclarationLocalServiceUtil
				.getLastByDocumentNameAndDocumentYear(documentName, documentYear);

		result.put("code", "NC_10");
		result.put("name", LanguageUtil.get(themeDisplay.getLocale(), "ban-khai-du-tru-cua-tau"));
		...
		result.put("messageType", MessageType.BAN_KHAI_DU_TRU_CUA_TAU);
		result.put("requestCode", requestCode);

		String icon = checkDisplayIcon(documentName, documentYear, MessageType.BAN_KHAI_DU_TRU_CUA_TAU);
	...
	public int _banKhaiDuTruCuaTau(String requestCode, long documentName, int documentYear, int messageType,
		...
		try {
			...
			if (results != null && results.size() > 0) {
				...
				tempShipStoresDeclaration.setRequestState(desStatus);
