...
public class TempDangerousGoodsNanifestFinderImpl extends BasePersistenceImpl<TempDangerousGoodsNanifest> implements TempDangerousGoodsNanifestFinder {
	
	private Log log = LogFactoryUtil.getLog(TempDangerousGoodsNanifestFinderImpl.class);
	...
	public List<TempDangerousGoodsNanifest> findBydocumentNameAnddocumentYear(long documentName, int documentYear) {
		...
		try {
			...
			query.append("SELECT * FROM TEMP_DANGEROUS_GOODS_MANIFEST WHERE DocumentName = ? and DocumentYear = ? ");
			
			String sql = query.toString();
			log.debug("=========findBydocumentNameAnddocumentYear========" + sql);
			SQLQuery q = session.createSQLQuery(sql);
			q.setCacheable(false);
			q.addEntity("TempDangerousGoodsNanifest", TempDangerousGoodsNanifestImpl.class);
			QueryPos qPos = QueryPos.getInstance(q);
			qPos.add(documentName);
			qPos.add(documentYear);
			
			// execute the query and return a list from the db
			return (List<TempDangerousGoodsNanifest>) QueryUtil.list(q, getDialect(), QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			closeSession(session);
		}
		return new ArrayList<TempDangerousGoodsNanifest>();
	}
	...
	public int countBydocumentNameAnddocumentYear(long documentName, int documentYear) {
		...
		try {
			...
			query.append("SELECT count(*) as total FROM TEMP_DANGEROUS_GOODS_MANIFEST WHERE DocumentName = ? and DocumentYear = ? ");
	...
	public TempDangerousGoodsNanifest getLastByDocumentNameAndDocumentYear(long documentName, int documentYear) {
		...
		try {
			...
			query.append("SELECT * FROM temp_dangerous_goods_manifest WHERE DocumentName = ? and DocumentYear = ? order by ID desc");
			...
			// execute the query and return a list from the db
			List<TempDangerousGoodsNanifest> lstTem = (List<TempDangerousGoodsNanifest>) QueryUtil.list(q, getDialect(), QueryUtil.ALL_POS,
					QueryUtil.ALL_POS);
