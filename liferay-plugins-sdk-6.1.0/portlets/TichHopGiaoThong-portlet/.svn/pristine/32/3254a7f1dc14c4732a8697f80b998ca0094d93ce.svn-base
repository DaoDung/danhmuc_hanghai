package vn.gt.portlet.kehoach.tt4;
...
public class TT4JSONProvider {

	private static Log _log = LogFactoryUtil.getLog(TT4JSONProvider.class);
	...
	public static JSONArray getThanhPhanHoSo(ThemeDisplay themeDisplay, String documentType, long documentName,
		...
		JSONObject thongBaoTauDenCangObject = BanKhaiThongBaoUtils.buildThanhPhan(themeDisplay, documentName, documentYear, MessageType.THONG_BAO_TAU_DEN_CANG);
		JSONObject xacBaoDenCangObject = BanKhaiXacBaoUtils.buildThanhPhan(themeDisplay, documentName, documentYear, MessageType.XAC_BAO_TAU_DEN_CANG);
		JSONObject lenhDieuDongObject = BanKhaiLenhDieuDongUtils.buildThanhPhan(themeDisplay, documentName, documentYear);
		
		if (thongBaoTauDenCangObject.getBoolean("available")/*  && xacBaoDenCangObject.getBoolean("available")*/) {
...
//		result.put(BanKhaiHangHoaUtils.buildThanhPhan(themeDisplay, documentName, documentYear));
		result.put(thongBaoTauDenCangObject);
		result.put(xacBaoDenCangObject);
...
//		result.put(BanKhaiKiemDichDongVatUtils.buildThanhPhan(themeDisplay, documentName, documentYear));
	...
	public static JSONArray getFileThanhPhanVersionList(ThemeDisplay themeDisplay, int messageType, long documentName,
		...
		if (messageType == MessageType.BAN_KHAI_AN_NINH_TAU_BIEN) {
			...
			result = BanKhaiHangHoaUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			...
			result = BanKhaiDanhSachHangHoaNguyHiemUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			
		} else if (messageType == MessageType.BAN_KHAI_DU_TRU_CUA_TAU) {
			
			result = BanKhaiDuTruCuaTauUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			
		} else if (messageType == MessageType.BAN_KHAI_HANH_LY_THUYEN_VIEN) {
			
			result = BanKhaiHanhLyThuyenVienUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			
		} else if (messageType == MessageType.BAN_KHAI_BAO_Y_TE_HANG_HAI) {
			
			result = BanKhaiBaoYTeHangHaiUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			
		} else if (messageType == MessageType.BAN_KHAI_KIEM_DICH_THUC_VAT) {
			
			result = BanKhaiKiemDichThucVatUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			
		} else if (messageType == MessageType.BAN_KHAI_KIEM_DICH_DONG_VAT) {
			
			result = BanKhaiKiemDichDongVatUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
	...
	public static JSONObject exportPDFDetailThanhPhan(Locale locale, int documentName, int documentYear,
		...
		try {
			...
			long nanoTime = ReportsBusinessUtils.actionExport(requestCode, documentName, documentYear, messageType,
					ChuyenDichTrangThaiUtils.NHAP_CANH_DAU_KHI);
			...
			if (messageType == MessageType.BAN_KHAI_AN_NINH_TAU_BIEN) {
				...
				tenFileExport = nanoTime + "_" + ReportConstant.CargoDeclaration_EXPORT;
			} else if (messageType == MessageType.XAC_BAO_TAU_DEN_CANG) {
				...
				tenFileExport = nanoTime + "_" + ReportConstant.SHIP_STORES_DECLARATION_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_HANH_LY_THUYEN_VIEN) {
				tenFileExport = nanoTime + "_" + ReportConstant.CREW_EFFECTS_DECLARATION_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_BAO_Y_TE_HANG_HAI) {
				tenFileExport = nanoTime + "_" + ReportConstant.DECLARATION_OF_HEALTH_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_KIEM_DICH_THUC_VAT) {
				tenFileExport = nanoTime + "_" + ReportConstant.DECLARATION_FOR_PLANT_QUARANTINE_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_KIEM_DICH_DONG_VAT) {
				tenFileExport = nanoTime + "_" + ReportConstant.DECLARATION_FOR_ANIMAL_QUARANTINE_EXPORT;
			} else if (messageType == MessageType.GIAY_PHEP_ROI_CANG_CUOI_CUNG) {
...
//						}
			...
			resultShow = CheckBusinessUtil.checkRoleDisplayButton(ChuyenDichTrangThaiUtils.NHAP_CANH_DAU_KHI, messageType, vaiTro, temp, requestCode,
					actionRequest);
	...
	public static JSONObject getMessageType23EXT(ThemeDisplay themeDisplay, long documentName, int documentYear,
		...
		try {
			...
			ResultCompetion resultCompetion = ResultCompetionLocalServiceUtil.findByDocumentNameAndDocumentYear(documentName, documentYear);
