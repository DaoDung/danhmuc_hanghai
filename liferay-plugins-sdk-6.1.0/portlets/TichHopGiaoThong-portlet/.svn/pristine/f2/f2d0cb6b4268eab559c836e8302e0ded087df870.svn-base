...
public class TT3XuLyNghiepVuUtils {
	...
	public static int doActionHoSo(long documentName, int documentYear, int actionType, int orginStatus,
		...
		try {
			...
			if (desStatus == ChuyenDichTrangThaiUtils.KE_HOACH_CHO_CAP_LENH_DIEU_DONG) {
				...
				TT3TichHopMessageUtils.message_99_21(user.getEmailAddress(), uploadPortletRequest, actionRequest,
				...
				boolean resultMethod = TT3TichHopMessageUtils.message_99_22(user.getEmailAddress(),
				...
				if (desStatus == TrangThaiBanKhaiNhapCanh.CHO_TIEP_NHAN
					...
					boolean resultMethod = TT3TichHopMessageUtils.message_99_27(user.getEmailAddress(),
							uploadPortletRequest, actionRequest, documentName, documentYear, yKienReject);
				...
				boolean resultMethod = TT3TichHopMessageUtils.message_99_28(user.getEmailAddress(),
						uploadPortletRequest, actionRequest, documentName, documentYear, yKienReject);
				...
				// Tich hop send message 99-22
				boolean resultMethod = TT3TichHopMessageUtils.message_99_22(user.getEmailAddress(), uploadPortletRequest, actionRequest,
						documentName, documentYear, yKienReject);
				...
				boolean resultMethod = TT3TichHopMessageUtils.message_99_25(user.getEmailAddress(), uploadPortletRequest, actionRequest,
						documentName, documentYear, yKienReject);
				...
				TT3TichHopMessageUtils.message_99_23(user.getEmailAddress(), uploadPortletRequest, actionRequest,
						documentName, documentYear, yKienReject);
	...
	public static int doActionBanKhai(long documentName, int documentYear, int actionType, int orginStatus,
		...
		try {
			...
			if (messageType == MessageType.BAN_KHAI_AN_NINH_TAU_BIEN) {
				...
				BanKhaiGiayPhepQuaCangBoGTVTUtils action = new BanKhaiGiayPhepQuaCangBoGTVTUtils();
				
				result = action._giayPhepQuaCangGTVT(requestCode, documentName, documentYear, messageType, desStatus, user,
						uploadPortletRequest, actionRequest);
	...
	private static int thayDoiTrangThaiBanKhaiKeHoach(long documentName, int documentYear, int orginStatus,
		...
		lstDeclaration = ResultDeclarationLocalServiceUtil.findByDocumentNameAndBusinessTypeCodeAndDocumentYear(
				MessageType.THONG_BAO_TAU_QUA_CANH, tempDocument.getDocumentName(), tempDocument.getDocumentYear());
		if (lstDeclaration != null && lstDeclaration.size() > 0) {
			ResultDeclaration resultDeclaration = lstDeclaration.get(0);
			TempNoTiceShipMessage tempNoticeShipMessage = TempNoTiceShipMessageLocalServiceUtil
					.findTempNoTiceShipMessageByRequestCode(resultDeclaration.getRequestCode());
			...
			BusinessUtils.updateResultDeclaration(MessageType.THONG_BAO_TAU_QUA_CANH,
					tempDocument.getDocumentName(), tempDocument.getDocumentYear(),
					trangThaiBanKhai);
		...
		lstDeclaration = ResultDeclarationLocalServiceUtil.findByDocumentNameAndBusinessTypeCodeAndDocumentYear(
				MessageType.XAC_BAO_TAU_QUA_CANH, tempDocument.getDocumentName(), tempDocument.getDocumentYear());
		if (lstDeclaration != null && lstDeclaration.size() > 0) {
			ResultDeclaration resultDeclaration = lstDeclaration.get(0);
			TempNoTiceShipMessage tempNoticeShipMessage = TempNoTiceShipMessageLocalServiceUtil
					.findTempNoTiceShipMessageByRequestCode(resultDeclaration.getRequestCode());
			...
			BusinessUtils.updateResultDeclaration(MessageType.XAC_BAO_TAU_QUA_CANH,
					tempDocument.getDocumentName(), tempDocument.getDocumentYear(),
					trangThaiBanKhai);
