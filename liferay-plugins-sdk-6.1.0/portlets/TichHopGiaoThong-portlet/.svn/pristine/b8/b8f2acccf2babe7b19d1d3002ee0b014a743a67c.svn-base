...
public class BanKhaiDanhSachHanhKhachUtils {
	...
	public static JSONObject buildThanhPhan(ThemeDisplay themeDisplay, long documentName, int documentYear) {
		...
		TempPassengerList results = TempPassengerListLocalServiceUtil.getLastByDocumentNameAndDocumentYear(documentName,
				documentYear);

		result.put("code", "NC_8");
		result.put("name", LanguageUtil.get(themeDisplay.getLocale(), "danh-sach-hanh-khach"));
		...
		result.put("messageType", MessageType.BAN_KHAI_DANH_SACH_HANH_KHACH);
		result.put("requestCode", requestCode);

		String icon = checkDisplayIcon(documentName, documentYear, MessageType.BAN_KHAI_DANH_SACH_HANH_KHACH);
	...
	public static JSONArray getFileThanhPhanVersionList(ThemeDisplay themeDisplay, long documentName, int documentYear,
		...
		try {
			...
			lstDeclaration = ResultDeclarationLocalServiceUtil.findByDocumentNameAndBusinessTypeCodeAndDocumentYear(
					MessageType.BAN_KHAI_DANH_SACH_HANH_KHACH, documentName, documentYear);
	...
	public int _banKhaiDanhSachHanhKhach(String requestCode, long documentName, int documentYear, int messageType,
		...
		try {
			...
			if (passengerListes != null && passengerListes.size() > 0) {
				...
				if (desStatus == ChuyenDichTrangThaiUtils.BAN_KHAI_CHAP_NHAN_BAN_KHAI) {
					...
					// Tich hop send message 52-22
					if (tempDocument.getDocumentTypeCode().equalsIgnoreCase("NC")) {
						boolean resultMethod = TT1TichHopMessageUtils.message_52_22(user.getEmailAddress(),
				...
				passenger.setRequestState(desStatus);
