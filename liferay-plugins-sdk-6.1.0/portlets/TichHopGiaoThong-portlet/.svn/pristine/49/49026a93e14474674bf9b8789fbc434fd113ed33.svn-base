		...
		<div class="tbl_left boxlabel">
			<table class="table_noboder">
				<thead>
						...
						<td class="text-leftu">
							<select name="documentStatusCode" id="documentStatusCode" style="width: 98%;">
								...
								<%for (DmGtStatus item: gtStatus) {%>
									<option 
									<%=(item.getStatusCode() == FormatData.convertToInt(documentStatusCode)) ? "selected" : ""%>
									<%-- <%=FormatData.checkedData(String.valueOf(item.getStatusCode()), documentStatusCode) %> --%>
					...
					<% if (MessageType.LOAT_THU_TUC_DANG_DI_CHUYEN == menuLoaiThuTuc) {%>
						<tr>
...
$(document).ready(function() {
	...
	function changeDmGtStatus(){				
		...
		// Thu tuc - Nhap canh, Vao cang bo 'De nghi cap giay phep, De nghi sua giay phep'
		if(menuThuTuc == ttNc || menuThuTuc == ttVc){
			$('#documentStatusCode option').each(function(){
				var statusCode = $(this).val();
				var statusName = $(this).text();					
							
				if(statusCode == 20 || statusCode == 120){													
					$(this).hide();
				}else{
					$(this).show();			
				}			
