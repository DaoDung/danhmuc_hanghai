...
/**
 * The base implementation of the temp declaration for animal quarantine local service.
 *
 * <p>
 * This implementation exists only as a container for the default service methods generated by ServiceBuilder. All custom service methods should be put in {@link vn.gt.dao.noticeandmessage.service.impl.TempDeclarationForAnimalQuarantineLocalServiceImpl}.
 * </p>
 *
 * @author win_64
...
public abstract class TempDeclarationForAnimalQuarantineLocalServiceBaseImpl
	 ...
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use {@link vn.gt.dao.noticeandmessage.service.TempDeclarationForAnimalQuarantineLocalServiceUtil} to access the temp declaration for animal quarantine local service.
	 */
	...
	public TempDeclarationForAnimalQuarantine addTempDeclarationForAnimalQuarantine(
		...
		tempDeclarationForAnimalQuarantine = tempDeclarationForAnimalQuarantinePersistence.update(tempDeclarationForAnimalQuarantine,
				false);
		...
		if (indexer != null) {
			try {
				indexer.reindex(tempDeclarationForAnimalQuarantine);
			}
	...
	public void deleteTempDeclarationForAnimalQuarantine(long id)
		...
		TempDeclarationForAnimalQuarantine tempDeclarationForAnimalQuarantine = tempDeclarationForAnimalQuarantinePersistence.remove(id);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.delete(tempDeclarationForAnimalQuarantine);
			}
			catch (SearchException se) {
				if (_log.isWarnEnabled()) {
					_log.warn(se, se);
	...
	public void deleteTempDeclarationForAnimalQuarantine(
		TempDeclarationForAnimalQuarantine tempDeclarationForAnimalQuarantine)
		throws SystemException {
		tempDeclarationForAnimalQuarantinePersistence.remove(tempDeclarationForAnimalQuarantine);
	...
	public TempDeclarationForAnimalQuarantine updateTempDeclarationForAnimalQuarantine(
		TempDeclarationForAnimalQuarantine tempDeclarationForAnimalQuarantine)
		throws SystemException {
		return updateTempDeclarationForAnimalQuarantine(tempDeclarationForAnimalQuarantine,
			true);
	}
	...
	public void afterPropertiesSet() {
		PersistedModelLocalServiceRegistryUtil.register("vn.gt.dao.noticeandmessage.model.TempDeclarationForAnimalQuarantine",
	...
	private static Log _log = LogFactoryUtil.getLog(TempDeclarationForAnimalQuarantineLocalServiceBaseImpl.class);
