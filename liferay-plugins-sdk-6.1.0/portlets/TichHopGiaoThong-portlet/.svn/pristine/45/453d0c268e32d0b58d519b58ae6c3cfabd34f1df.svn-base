	...
	public ShiftingOrder insert2ShiftingOrder(long documentName, int  documentYear) throws Exception {
		...
		if (object != null) {
			try {
				...
				List<IssueShiftingOrderChanel> listShiftingOrderChanels = IssueShiftingOrderChanelLocalServiceUtil.getShiftingOrderChanel(object.getPrimaryKey());
				...
				if(listShiftingOrderChanels != null) {
					for (IssueShiftingOrderChanel shiftingOrderChanel : listShiftingOrderChanels) {
						...
						chanel.setChanelCode(shiftingOrderChanel.getChanelCode());
