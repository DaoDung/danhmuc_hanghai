...
public class ProcessSendMessage implements Runnable {
			...
			while (true) {
				try {
					...
					if(messageQueue != null) {
						_log.info("===messageQueue===ProcessSendMessage===" + messageQueue.getId() + "===" + sb.toString());
	...
	private void sendMessage(AswmsgMessageQueue messageQueue) {
		...
		try {
			//_log.info("===messageQueue===AllContent===" + messageQueue.getAllContent());
	...
	private String sendWS(AswmsgMessageQueue messageQueue, TempDocument tempDocument) throws Exception {
		...
		if (messageQueue != null && tempDocument != null 
			...
			int[] thuTucInland = new int[] {MessageType.TAU_VAO, MessageType.TAU_RA, MessageType.CHUYEN_CANG_VAO, MessageType.CHUYEN_CANG_ROI};
			...
			// Gui message toi CVHH khi nhan duoc message tu VNSW
			if(messageQueue.getValidationCode().equalsIgnoreCase(MessageType.REQUEST_DIRECTION_OUT_CVHH)) {
				...
				if (ArrayUtil.contains(messageTypeSendCVHH, messageType)) {
					...
					//_log.info("===SEND_CVHH===" + allContent);
					returnXML = sendCVHH(headerRequest, allContent, tempDocument);
				} else {
					// khong can phai gui di, xoa trong messagequeue
					returnXML = String.valueOf(messageType);
				}
			} else if(messageQueue.getValidationCode().equalsIgnoreCase(MessageType.REQUEST_DIRECTION_CV_TO_HQ)) {
				...
				if (ArrayUtil.contains(thuTucMT, documentType)) {		// SEND VNSW
					...
					_log.info("===Process_Send_Messsage===returnXML===MT===" + returnXML);
					...
					_log.info("===Process_Send_Messsage===returnXML===Inland===" + returnXML);
					...
					_log.info("===Process_Send_Messsage===returnXML===VIWA===" + returnXML);
				}			
	...
	private String sendCVHH(Header header, String allContent, TempDocument tempDocument) 
			...
			for(DmGtRouteConfig routeConfig : routeConfigs) {
				...
				if(vnmwsSoap != null) {
					...
					try {
						...
						_log.info("===routeConfigCode===" + routeConfig.getRouteCode());
						_log.error("===SEND_CVHH_ERROR===" + e.getMessage());
					}
				} else {
					_log.info("NOT_CONNECTED_WEBSERVICE: VNMWSSoap is null");
				}
				
				_log.debug("===returnXML===" + returnXML);
