...
public class TT4ActionProvider {

	private static Log _log = LogFactoryUtil.getLog(TT4ActionProvider.class);
	...
	public static JSONObject actionKeHoach(ThemeDisplay themeDisplay, UploadPortletRequest uploadPortletRequest,
		...
		if (chuyenDichStatus != ChuyenDichTrangThaiUtils.CHUYEN_DICH_THAT_BAI) {
			...
			if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
				int state = TT4XuLyNghiepVuUtils.doActionHoSo(documentName, documentYear, actionType, requestState,
				...
				if (state == TT4XuLyNghiepVuUtils.XU_LY_THAT_BAI) {
					ChuyenDichTrangThaiUtils.doHoSo(documentName, documentYear, requestState, Boolean.TRUE);
				}
			} else {
				TT4XuLyNghiepVuUtils.doActionBanKhai(documentName, documentYear, actionType, requestState,
	...
	public static JSONObject actionThuTuc(ThemeDisplay themeDisplay, UploadPortletRequest uploadPortletRequest,
		...
		if (chuyenDichStatus != ChuyenDichTrangThaiUtils.CHUYEN_DICH_THAT_BAI) {
			...
			if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
				...
				chuyenDichStatus = TT4XuLyNghiepVuUtils.doActionBanKhai(documentName, documentYear, actionType,
			...
			_log.info("chuyenDichStatus _doActionBanKhai: " + chuyenDichStatus);
...
//				}
				...
				if (Validator.isNotNull(tempDebitNote) && tempDebitNote.getTotalpayment() > 0) {
					...
					boolean resultMethod = TT4TichHopMessageUtils.message_99_30(themeDisplay.getUser().getEmailAddress(),
