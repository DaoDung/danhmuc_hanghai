...
import vn.gt.portlet.kehoach.nghiepvu.bankhai.BanKhaiGiayPhepQuaCangBoGTVTUtils;
import vn.gt.portlet.kehoach.nghiepvu.bankhai.BanKhaiGiayPhepQuaCangCuoiCungUtils;
...
public class TT3JSONProvider {

	private static Log _log = LogFactoryUtil.getLog(TT3JSONProvider.class);
	...
	public static JSONArray getThanhPhanHoSo(ThemeDisplay themeDisplay, String documentType, long documentName,
		...
		JSONObject thongBaoTauQuaCangObject = BanKhaiThongBaoUtils.buildThanhPhan(themeDisplay, documentName, documentYear, MessageType.THONG_BAO_TAU_QUA_CANH);
		JSONObject xacBaoQuaCanhObject = BanKhaiXacBaoUtils.buildThanhPhan(themeDisplay, documentName, documentYear, MessageType.XAC_BAO_TAU_QUA_CANH);
		JSONObject lenhDieuDongObject = BanKhaiLenhDieuDongUtils.buildThanhPhan(themeDisplay, documentName, documentYear);
		
		if (thongBaoTauQuaCangObject.getBoolean("available")/*  && xacBaoQuaCanhObject.getBoolean("available")*/) {
		...
		result.put(thongBaoTauQuaCangObject);
		result.put(xacBaoQuaCanhObject);
		...
		try {
			...
			if (isThuTuc && roleUserFilterselectedType != -1) {
				...
				result.put(BanKhaiGiayPhepQuaCangBoGTVTUtils.buildThanhPhan(themeDisplay, documentName, documentYear));
			}
		...
		JSONObject lastLost = BanKhaiGiayPhepQuaCangCuoiCungUtils.buildThanhPhan(themeDisplay, documentName, documentYear);
	...
	public static JSONArray getFileThanhPhanVersionList(ThemeDisplay themeDisplay, int messageType, long documentName,
		...
		if (messageType == MessageType.BAN_KHAI_AN_NINH_TAU_BIEN) {
			...
			result = BanKhaiAnNinhTauBienUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			...
			result = BanKhaiGiayPhepQuaCangBoGTVTUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear, messageType);
			
		} else if (messageType == MessageType.GIAY_PHEP_ROI_CANG_CUOI_CUNG) {

			result = BanKhaiGiayPhepQuaCangCuoiCungUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);

		} else if (messageType == MessageType.BAN_KHAI_DANH_SACH_HANG_HOA_NGUY_HIEM) {

			result = BanKhaiDanhSachHangHoaNguyHiemUtils.getFileThanhPhanVersionList(themeDisplay, documentName, documentYear,
					messageType);
	...
	public static JSONObject exportPDFDetailThanhPhan(Locale locale, int documentName, int documentYear,
		...
		try {
			...
			long nanoTime = ReportsBusinessUtils.actionExport(requestCode, documentName, documentYear, messageType,
					ChuyenDichTrangThaiUtils.TAU_THUYEN_QUA_CANH);
			...
			if (messageType == MessageType.GIAY_PHEP_QUA_CANH) {
				issuePermissionForTransit = IssuePermissionForTransitLocalServiceUtil.getByrequestCode(requestCode);
			}
			
			if (messageType == MessageType.BAN_KHAI_AN_NINH_TAU_BIEN) {
				tenFileExport = nanoTime + "_" + ReportConstant.ShipSecurityNotification_EXPORT;
			} else if (messageType == MessageType.XAC_BAO_TAU_QUA_CANH) {
				tenFileExport = nanoTime + "_" + ReportConstant.XAC_BAO_TAU_QUA_CANH_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_DANH_SACH_HANG_HOA_NGUY_HIEM) {
				tenFileExport = nanoTime + "_" + ReportConstant.DANGEROUS_GOODS_MANIFEST_EXPORT;
			} else if (messageType == MessageType.THONG_BAO_TAU_QUA_CANH) {
				tenFileExport = nanoTime + "_" + ReportConstant.THONG_BAO_TAU_QUA_CANH_EXPORT;
			} else if (messageType == MessageType.LENH_DIEU_DONG) {
				...
				if ( Validator.isNotNull(issuePermissionForTransit) && issuePermissionForTransit.getAttachedFile() > 0 ) {
					tenFileExport = vn.gt.portlet.document.FileUtils.getFullFileURL(issuePermissionForTransit.getAttachedFile());
				} else {
					tenFileExport = nanoTime + "_" + ReportConstant.PERMISSION_FOR_TRANSIT_EXPORT;
				}
			} else if (messageType == MessageType.GIAY_PHEP_ROI_CANG_CUOI_CUNG) {
				...
				if (Validator.isNotNull(issuePortClearance)) {
					if (issuePortClearance.getAttachedFile() > 0 && DocumentUtils.existDLFileAbsPath(issuePortClearance.getAttachedFile())) {
						...
						_log.info("-------getRequestCode+++++++++++" + issuePortClearance.getRequestCode());
			...
			if ( (Validator.isNotNull(shiftOrder) && shiftOrder.getAttachedFile() > 0 && messageType == MessageType.LENH_DIEU_DONG) ||
					(Validator.isNotNull(issuePermissionForTransit) && issuePermissionForTransit.getAttachedFile() > 0 && messageType == MessageType.GIAY_PHEP_QUA_CANH)) {
...
//						}
				...
				if (messageType == 90) {
					...
					List<IssuePermissionForTransit> lstIssuePortClearance = IssuePermissionForTransitLocalServiceUtil
							.findByDocumentYearAndDocumentYearOrderByColumn(documentName, documentYear,
									KeyParams.ID, KeyParams.ORDER_BY_DESC);

					if (lstIssuePortClearance.size() > 0) {
//						for (IssuePortClearance item : lstIssuePortClearance) {
							if (lstIssuePortClearance.get(0).getRequestState() == TrangThaiBanKhaiNhapCanh.KHAI_SUA && lstIssuePortClearance.get(0).getRequestState() == ChuyenDichTrangThaiUtils.THU_TUC_DE_NGHI_SUA_GIAY_PHEP) {
								resultShow = 2;
//								break;
							}
//						}
			...
			resultShow = CheckBusinessUtil.checkRoleDisplayButton(ChuyenDichTrangThaiUtils.TAU_THUYEN_QUA_CANH, messageType, vaiTro, temp, requestCode,
					actionRequest);
