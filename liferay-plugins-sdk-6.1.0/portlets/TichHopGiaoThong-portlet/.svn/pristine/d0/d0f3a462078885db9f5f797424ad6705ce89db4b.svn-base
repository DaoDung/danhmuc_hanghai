...
//			log.info("===========To--OrganizationCode--" + header.getTo().getOrganizationCode());
	...
	public boolean insertHistoryReceiveMessageResponse(String dataInput) {
		...
		try {
			Header header = null;
			log.info("===insertHistoryReceiveMessageResponse==Thong tin nhan tu HQMC===" + dataInput);
			String data = replaceXML(dataInput.trim());
	...
	public String createContentSendFromBGTVTToNSW(Header header, String xmlInnerTagContent) {
		...
		try {
			/*
			 * Messsage messsage = new Messsage(String.valueOf(header.getReference().getVersion()), UUID.randomUUID().toString(),
			 * header.getFrom().getName(), String.valueOf(header.getFrom().getIdentity()), header.getTo().getName(),
			 * String.valueOf(header.getTo().getIdentity()), String.valueOf(header.getSubject().getDocumentType()),
			 * FormatData.formatIntToString(header.getSubject().getType()), String.valueOf(header.getSubject().getFunction()),
			 * String.valueOf(header.getSubject().getReference()), String.valueOf(header.getSubject().getPreReference()),
			 * String.valueOf(header.getSubject().getDocumentYear()), header.getSubject().getSendDate());
			 */
	...
	public String createContentSendFromBGTVTToNSWInland(Header header, String xmlInnerTagContent) {
		String xmlResult = createContentSendFromBGTVTToNSW(header, xmlInnerTagContent);
		/*try {
			...
			MesssageInland messsageinland = new MesssageInland(ref_version, ref_messageId, frm_name, frm_identity, to_name, to_identity,
					documentType, type, function, reference, preReference, documentYear, sendDate, frm_CountryCode, frm_MinistryCode,
					frm_OrganizationCode, frm_UnitCode, to_CountryCode, to_MinistryCode, to_OrganizationCode, to_UnitCode);
	...
	public IssueShiftingOrder lenhDieuDongUpgrade(UploadPortletRequest actionRequest, long documentName, int documentYear, String username, String requestCode, long requestState) {
		...
		try {
			...
			if (Validator.isNotNull(listIssueShiftingOrder) && listIssueShiftingOrder.size() > 0) {
				...
				shiftOrder = new IssueShiftingOrderImpl();
				isAdd = true;
			}
			...
			double shownDraftxF = ParamUtil.getDouble(actionRequest, "shownDraftxF");
			double shownDraftxA = ParamUtil.getDouble(actionRequest, "shownDraftxA");
			
			String from = ParamUtil.getString(actionRequest, "from");
			double dwt = ParamUtil.getDouble(actionRequest, "dwt");
			double loa = ParamUtil.getDouble(actionRequest, "loa");
			...
			if (Validator.isNull(dmPortRegion)) { dmPortRegion = new DmPortRegionImpl(); }
...
//			cang bien + portRegin + portHauboru + ShiftingPortWharfCode
			...
			for (int i = 0; i < chanelArray.length(); i++) {
				...
				issueShiftingOrderChanel.setShiftingOrderId(shiftOrder.getId());
				issueShiftingOrderChanel.setChanelCode(chanelObject.getString("code0"));
	...
	public String getDivision(ActionRequest resourceRequest) {
		...
		try {
			...
			String organizationId = (String) resourceRequest.getPortletSession().getAttribute(PageType.ORGANIZATION_TYPE,
	...
	public String _processMessage(Header header, String requestCode, String data) {
		...
		SendMessgaeFunctions.insertMessageQueue(header, xmlResult, "OUT", requestCode, MessgaePriorityLevels.HIGHT);
