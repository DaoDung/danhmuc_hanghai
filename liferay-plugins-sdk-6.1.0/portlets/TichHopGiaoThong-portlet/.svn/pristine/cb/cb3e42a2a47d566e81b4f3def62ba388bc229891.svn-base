...
import vn.gt.dao.danhmucgt.service.base.DmGtRouteConfigLocalServiceBaseImpl;
...
public class DmGtRouteConfigLocalServiceImpl
	extends DmGtRouteConfigLocalServiceBaseImpl {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	...
	public List<DmGtRouteConfig> findByIsDelete(int isDelete) {
		try {
			return dmGtRouteConfigPersistence.findByIsDelete(isDelete);
	...
	public List<DmGtRouteConfig> findByLocCode(String locCode) {
		try {
			return dmGtRouteConfigPersistence.findByLocCode(locCode, 0);
		} catch (SystemException e) {
			_log.error(e);
		}
	...
	private static Log _log = LogFactoryUtil.getLog(DmGtRouteConfigLocalServiceImpl.class);
}