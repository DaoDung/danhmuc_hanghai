...
String key_previous = "previous";
String key_next = "next";
String key_first = "first"; 
String key_last = "last";
String key_page = "page";
String key_items_per_page = "items-per-page";
String key_of = "of";

if(type.equals("en")) {
	key_previous = "en-previous";
	key_next = "en-next";
	key_first = "en-first"; 
	key_last = "en-last";
	key_page = "en-page";
	key_items_per_page = "en-items-per-page";
	key_of = "en-of";
}
%>

<c:if test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") || type.equals("en") || (type.equals("article") && (total > resultRowsSize)) %>'>
	<div class="taglib-page-iterator">
</c:if>

<c:if test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") || type.equals("en") %>'>
...
<c:if test="<%= (total > delta) || (total > PropsValues.SEARCH_CONTAINER_PAGE_DELTA_VALUES[0]) %>">
	<div class="search-pages">
		<c:if test='<%= type.equals("regular") || type.equals("en") %>'>
				...
				<div class="delta-selector">
					<c:choose>
						<c:when test="<%= !deltaConfigurable || themeDisplay.isFacebook() %>">
							<liferay-ui:message key="<%= key_items_per_page%>" />

							<%= delta %>
						</c:when>
						<c:otherwise>
							<aui:select changesContext="<%= true %>" inlineLabel="left" label="<%= key_items_per_page %>" name="itemsPerPage" onchange='<%= namespace + deltaParam + "updateDelta(this);" %>'>
			...
			<div class="page-selector">
				<c:choose>
					<c:when test="<%= themeDisplay.isFacebook() %>">
						<liferay-ui:message key="<%= key_page%>" />
					...
					<c:otherwise>
						...
						<%
						String suffix = LanguageUtil.get(pageContext, key_of) + StringPool.SPACE + numberFormat.format(pages);
						...
						<aui:select changesContext="<%= true %>" inlineLabel="left" label="<%= key_page %>" name="page" onchange='<%= namespace + curParam + "updateCur(this);" %>' suffix='<%= suffix %>'>
		...
		<div class="page-links">
			<c:if test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") || type.equals("en") %>'>
				...
				<liferay-ui:message key="<%= key_first%>" />
			...
			<c:choose>
				...
				<c:when test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") || type.equals("en") %>'>
					<span class="previous">
				</c:when>
			</c:choose>

			<c:if test='<%= (type.equals("approximate") || type.equals("more") || type.equals("regular") || cur != 1) %>'>
				<liferay-ui:message key="<%= key_previous %>" />
			</c:if>
			...
			<c:if test='<%= (type.equals("approximate") || type.equals("more") || type.equals("regular") || type.equals("en") || cur != pages) %>'>
				<c:choose>
					<c:when test='<%= type.equals("approximate") || type.equals("more") %>'>
						<liferay-ui:message key="more" />
					</c:when>
					<c:otherwise>
						<liferay-ui:message key="<%= key_next %>" />
					</c:otherwise>
				</c:choose>
			</c:if>
			...
			<c:if test='<%= type.equals("regular") || type.equals("en") %>'>
				...
				<liferay-ui:message key="<%= key_last%>" />
...
<c:if test='<%= type.equals("approximate") || type.equals("more") || (type.equals("regular") || type.equals("en")) && !themeDisplay.isFacebook() %>'>
	<aui:script>
		Liferay.provide(
			window,
