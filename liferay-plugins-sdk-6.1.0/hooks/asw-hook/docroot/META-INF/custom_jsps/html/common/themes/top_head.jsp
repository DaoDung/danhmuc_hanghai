...
%>
	...
	<c:if test="<%= locale.equals(defaultLocale) %>">
		...
		%>
						...
						<link href="<%= HtmlUtil.escapeAttribute(PortalUtil.getAlternateURL(request, canonicalURL, curLocale)) %>" hreflang="<%= LocaleUtil.toW3cLanguageId(curLocale) %>" rel="alternate" />
...
<link href="<%= HtmlUtil.escapeAttribute(PortalUtil.getStaticResourceURL(request, themeDisplay.getCDNHost() + themeDisplay.getPathContext() + "/html/css/main.css")) %>" rel="stylesheet" type="text/css" />
